...
class Numero{
	...
	calcularCaracteristica(pEnteraBinario, pDecimalBinario, esDoble){
		...
		if(pEnteraBinario != '') {
			...
			// comprobamos si el numero es desnormalizado
			if(!esDoble){
				if (i >= 128) return "00000000";
			}
			else{
				if (i >= 1024) return "00000000000";
			}
		}
	...
	constructor(numeroEnTexto, esPos, deDondeV){
		...
		if(true){
			if(!this.igualesCerosBinario(numeroEnTexto)){
				...
				if(numeroEnTexto != "" && (this._caracteristicaSencilla == "00000000") ){
				...
				if(numeroEnTexto != "" && (this._caracteristicaDoble == "00000000000") ){
